...
# HA Smart Context — generates an AGENTS.md with Home Assistant context
# Codewhale auto-loads AGENTS.md from the workspace root (/config), giving every session HA awareness
#
# Usage:
...
OUTPUT_FILE="${OUTPUT_FILE:-/config/AGENTS.md}"
FULL_MODE=false
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help)
            ...
            echo ""
            echo "Generate Home Assistant context for Codewhale sessions."
            echo "Writes an AGENTS.md file that Codewhale automatically loads."
            echo ""
            echo "Options:"
            echo "  --full       Include entity ID listings (detailed mode)"
            echo "  --output F   Write to file F instead of /config/AGENTS.md"
...
> Auto-generated by Codewhale Terminal add-on. Run \`ha-context\` to refresh.
